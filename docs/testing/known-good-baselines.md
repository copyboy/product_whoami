...
| ---- | ------ | --------- | ----- | --------------- | -------------- | -------- | ---- |
| 2026-09-09 | local | dirty tree at run (WI3 changes, files named in Notes) | full | `npm run type-check` (exit 0); `npm run lint` (exit 0, 0 errors; 1 retained non-blocking warning `react-hooks/exhaustive-deps` at `SearchIsland.tsx:257`); `npm run build` (exit 0, 647 pages); `npm run test:run` (exit 0, 13 files / 120 tests, 含 `src/utils/` 57 个新用例：content 22 + seo 5 + web3Concepts 12 + web3Roadmap 18); `npm run verify` (exit 0 — type-check → lint → test:run → build → content-checks 全绿, content-checks 0 findings) | (none) | `docs/logs/2026/09-09.md` § quality-system/WI3; plan `docs/plans/quality-system/2026-09-09-0127-3-wi3-utils-unit-tests.md` § Verification | WI3 落地（L2 utils 单测防线）。Changed files at run: `src/utils/content.test.ts`、`src/utils/seo.test.ts`、`src/utils/web3Concepts.test.ts`、`src/utils/web3Roadmap.test.ts`（均为新建 colocated 测试，零生产代码改动）。Post-run docs-only changes: this row, `docs/logs/2026/09-09.md`, plan checkbox ticks, `docs/backlog/quality-system-roadmap.md` WI3 tick. |
